...
// Inlined from the canvasui shared rect-cache module (not shipped in the
// registry item): a cached bounding-rect that stays fresh via ResizeObserver.
function createRectCache(element) {
  const state = { current: element.getBoundingClientRect() };
...
function sampleAlbedo(data, u, v, flipY, out) {
  const x = Math.min(
    data.width - 1,
...
function sampleMesh(scene, count) {
  ...
  scene.traverse((node) => {
    ...
    const positions2 = geometry.getAttribute("position");
    if (!positions2) return;
    const index = geometry.getIndex();
    const triangleCount = Math.floor(
      (index ? index.count : positions2.count) / 3
    );
  ...
  let totalArea = 0;
  const vertexIndex = (bucket, tri, corner) => bucket.index ? bucket.index.getX(tri * 3 + corner) : tri * 3 + corner;
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (bucket.vertexColors) {
      albedo.multiplyScalar(
        (bucket.vertexColors.getX(i0) * w + bucket.vertexColors.getX(i1) * u + bucket.vertexColors.getX(i2) * v + bucket.vertexColors.getY(i0) * w + bucket.vertexColors.getY(i1) * u + bucket.vertexColors.getY(i2) * v + bucket.vertexColors.getZ(i0) * w + bucket.vertexColors.getZ(i1) * u + bucket.vertexColors.getZ(i2) * v) / 3
      );
    }
    let shade = 0.85;
    if (bucket.normals) {
...
export function createParticleObject(elements, options = {}) {
  ...
  function onPointerMove(event) {
    ...
    if (pointerActive && lastPointerTime) {
      const dt = Math.max((now - lastPointerTime) / 1e3, 1e-3);
  ...
  function simulate(delta) {
    ...
    if (pointerActive && !reducedMotion && config.strength > 0) {
      ...
      const worldPerPx = 2 * camera.position.distanceTo(floatGroup.position) * Math.tan(THREE.MathUtils.degToRad(camera.fov) / 2) / height;
  ...
  function tick(time) {
    ...
    const delta = lastTime ? Math.min((time - lastTime) / 1e3, 1 / 30) : 0;
    lastTime = time;
    controls.update();
    if (!reducedMotion) {
