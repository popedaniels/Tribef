...
export default function SuspendCampaignModal({
  onModalClose,
  showModal,
  campaign,
}) {
  ...
  const handleSuspendCampaign = async () => {
    ...
    try {
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/suspendCampaign/${campaign._id}`,
        {
    ...
    >
      <Wrapper className="d-flex flex-column p-5">
        {step === 1 ? (
          <>
            {" "}
            <h2 className="text-medium-heading mb-3">Suspend Campaign</h2>
          ...
          <div className="d-flex flex-column">
            ...
            <p className="text-small mb-3">
              ...
              why the "${campaign?.basicInformation?.campaignTitle}" campaign is being suspended?`}
            </p>
            <textarea
              name="message"
              id="message"
            ...
            <div className="d-flex ml-auto mt-5">
              ...
              >
                {loading ? (
                  ...
                  <p className="mb-0 text-white">Suspend</p>
                )}
              </button>
            </div>
          </div>
