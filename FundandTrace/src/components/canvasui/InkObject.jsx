...
function buildShapes(canvas, aspectW, aspectH) {
  ...
  const rings = traceContours(inside, width, height).map((points) => simplify(points, SIMPLIFY_TOLERANCE)).filter((points) => points.length >= 6 && ringArea(points) >= MIN_AREA).map((points) => ({ points, area: ringArea(points), depth: 0 })).sort((a, b) => b.area - a.area).slice(0, MAX_CONTOURS);
  ...
  for (const ring of rings) {
    ...
    const shape = parent ? shapes.get(parent) : void 0;
...
function createImageObject(canvas, anisotropy, lit, depth) {
  ...
  const material = lit ? new THREE.MeshStandardMaterial({
    map: texture,
    roughness: 0.6,
    metalness: 0
...
export function createInkObject(elements, options = {}) {
  ...
  function refreshEnvironment() {
    ...
    envTarget?.dispose();
    envTarget = pmrem.fromScene(roomScene, 0, 0.1, 1e3);
  ...
  function applyRoughness() {
    ...
    model.traverse((node) => {
      ...
      for (const material of materials) {
        ...
        if (standard.userData.baseRoughness === void 0) {
        ...
        // three r149 has no Scene.environmentIntensity (r163+); apply the
        // config on the material instead so the env doesn't wash the ink tone out
        if (standard.envMapIntensity !== void 0) {
          standard.envMapIntensity = config.environmentIntensity;
        }
      }
    });
  }
  ...
  function buildImageModel() {
    ...
    builtDepth = Math.min(Math.max(config.depth, 2e-3), 1);
    adoptModel(
      createImageObject(
        imageSource.canvas,
  ...
  function applyOptions() {
    ...
    if (imageSource && Math.min(Math.max(config.depth, 2e-3), 1) !== builtDepth) {
      buildImageModel();
    }
    ...
    uniforms.uHeightSpan.value = Math.max(config.scale, 1e-3) * 0.5;
  ...
  function tick(time) {
    ...
    uniforms.uHeightCenter.value = -modelCenter.copy(floatGroup.position).applyMatrix4(camera.matrixWorldInverse).z;
