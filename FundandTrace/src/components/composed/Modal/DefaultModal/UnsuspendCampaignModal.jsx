...
export default function UnsuspendCampaignModal({
  onModalClose,
  showModal,
  campaign,
}) {
  ...
  const handleUnsuspendCampaign = async () => {
    setLoading(true);
    try {
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/unsuspendCampaign/${campaign._id}`,
        {
          organizerEmail: campaign?.organizerDetails?.email,
          message,
    ...
    >
      <Wrapper className="d-flex flex-column p-5">
        {step === 1 ? (
          <>
            {" "}
            <h2 className="text-medium-heading mb-3">Unsuspend Campaign</h2>
            <p className="text-small mb-5">
              Are you sure you want to unsuspend this campaign?
            </p>
          ...
          <div className="d-flex flex-column">
            ...
            <p className="text-small mb-3">
              {` Would you like to inform ${campaign?.organizer} the reasons
              why the "${campaign?.basicInformation?.campaignTitle}" campaign is being unsuspended?`}
            </p>
            <textarea
              name="message"
              id="message"
            ...
            <div className="d-flex ml-auto mt-5">
              <button
                ...
                onClick={() => handleUnsuspendCampaign()}
              >
                {loading ? (
                  <Spinner
                    type="TailSpin"
              ...
              >
                {loading ? (
                  ...
                  <p className="mb-0 text-white">Unsuspend</p>
                )}
              </button>
            </div>
          </div>
